...
ParseScript(string s)
{
    ...
    static map<string, ocnodetype> mapOpNames;

    if (mapOpNames.size() == 0)
    {
        for (int op = OP_NOP; op <= OP_NOP10; op++)
        {
            const char* name = GetOpName((ocnodetype)op);
            if (strcmp(name, "OP_UNKNOWN") == 0)
                continue;
            string strName(name);
            mapOpNames[strName] = (ocnodetype)op;
...
BOOST_AUTO_TEST_CASE(script_PushData)
{
    ...
    // the stack as the 1-75 ocnodes do.
